...
# TRAINER CAT IMPLEMENTATION        #
#####################################
# (existing TrainerCat code stays here)

#####################################
# ADDITIONAL TEST CATS              #
#####################################
...
class KANOrbitCat(Cat):
    """Tries to maintain a fixed distance from the player."""
    ...
    def move(self) -> None:
        ...
        # Calculate all possible moves and their resulting distances
        possible_moves = [
            (-1, 0), (1, 0), (0, -1), (0, 1), (0, 0)
        ]
        ...
        for dr, dc in possible_moves:
            ...
            new_distance = abs(new_r - self.player_pos[0]) + abs(new_c - self.player_pos[1])
            score = abs(new_distance - target_distance)
